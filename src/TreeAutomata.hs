...
--  trace ("eqGrammar': " ++ s1 ++ " " ++ s2) $
  case [r | r@(Right _, _) <- rs] of
    ...
    _ -> Nothing
  where
  rs = runStateT (runErrorT (f s1 s2)) []
  f :: Name -> Name -> ErrorT String (StateT [(Name, Name)] []) ()
  f s1 s2 = do
    mapping <- get
      ...
      Nothing -> do {-trace ("putting: " ++ s1 ++ " -> " ++ s2) $-}
                    ...
                    then let p1ctors = [c | (_, Ctor c _) <- p1]
                         ...
                         in throwError $ "Different number of productions: " ++ intercalate " !!!!! " [s1, s2, show $ length p1, show $ length p2, show $ zip p1ctors p2ctors, show p1ctors, show p2ctors, unlines $ map show p1, unlines $ map show p2]
                            ...
                            zipWithM_ g p1' p2'
  g :: Prod -> Prod -> ErrorT String (StateT [(Name, Name)] []) ()
...
{-
elimSelfLoop (Grammar s p) = Grammar s (filter f p) where
  f (l, Eps r) | l == r = False
  f _ = True
...
--Prods [("X","E"),("Y","E")] [("X",Prods [] [("X",Prods [("X","A"),("X","B"),("Y","A")] [])]),("Y",Prods [] [("X",Prods [("X","C"),("Y","C")] [])])]

{-
[
  ([],[("E",fromList ["X","Y"])]),
  ([fromList ["X"]],[]),
  ([fromList ["X"],fromList ["X"]],[("A",fromList ["X","Y"]),("B",fromList ["X"])]),
  ([fromList ["X"],fromList ["X"],fromList ["X"]],[]),
  ([fromList ["X"],fromList ["X"],fromList ["X","Y"]],[]),
  ([fromList ["X"],fromList ["X","Y"]],[("A",fromList ["X","Y"]),("B",fromList ["X"])]),
  ([fromList ["X"],fromList ["X","Y"],fromList ["X"]],[]),
  ([fromList ["X"],fromList ["X","Y"],fromList ["X","Y"]],[]),
  ([fromList ["X","Y"]],[]),
  ([fromList ["X","Y"],fromList ["X"]],[("A",fromList ["X","Y"]),("B",fromList ["X"]),("C",fromList ["X","Y"])]),
  ([fromList ["X","Y"],fromList ["X"],fromList ["X"]],[]),
  ([fromList ["X","Y"],fromList ["X"],fromList ["X","Y"]],[]),
  ([fromList ["X","Y"],fromList ["X","Y"]],[("A",fromList ["X","Y"]),("B",fromList ["X"]),("C",fromList ["X","Y"])]),
  ([fromList ["X","Y"],fromList ["X","Y"],fromList ["X"]],[]),
  ([fromList ["X","Y"],fromList ["X","Y"],fromList ["X","Y"]],[])]
-}
